...
cat(file = "./code/jags/mod1.jags", "
    ...
    ## Priors
    sigma.r ~ dunif(0, 10)               # Variance on log lambda for pop process model
    sigma2.r <- pow(sigma.r, 2)
    tau.r <- pow(sigma.r, -2)
    log.tau.r <- log(tau.r)
    
    for(i in 1:P){                               # For each plot (P),
        mu.r[i] ~ dnorm(0, 0.01)         # Avg. log lambda is normally dist. around zero
    ...
    for(i in 1:P){                               # For each plot,
        for(t in 2:T){                             # For years 2 to 28,
            r[i,t-1] <- mu.r[i] + eps.r[i,t-1]
            eps.r[i, t-1] ~ dnorm(0, tau.r)
            lambda[i,t-1] <- exp(r[i,t-1])
    ...
    ## Derived parameters
    for(i in 1:P){                                   # For each plot,
        mu.lambda[i] <- exp(mu.r[i])            # lambda is the exp. of mu.loglambda (i.e., r)
    } #i
    }
    ")
...
## Initial values
inits <- function() list(mu.r = runif(30, -0.5, 0.5),
                         sigma.r = runif(1, 0, 10),
...
pars <- c("N", "mu.r", "sigma.r", "mu.lambda", "lambda", "p")
